...
class TestSetup
{
    ...
    public static function setupProjectConfig(string $projectConfigFolder = null)
    {
        if (!$projectConfigFolder) {
            ...
            $projectConfigFolder = dirname(CRAFT_TESTS_PATH).DIRECTORY_SEPARATOR.'config'.DIRECTORY_SEPARATOR.$config['folder'];
        }
        ...
        $dest = CRAFT_CONFIG_PATH.DIRECTORY_SEPARATOR.$config['folder'];

        // Clear any existing.
        if (is_dir($dest)) {
            FileHelper::removeDirectory($dest);
        }

        // Copy the data over.
        FileHelper::copyDirectory($projectConfigFolder, CRAFT_CONFIG_PATH.DIRECTORY_SEPARATOR.$config['folder']);
    }
